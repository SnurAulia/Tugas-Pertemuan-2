...
public function insert($kd_barang, $nm_barang, $harga, $jml) 
{
    ...
    $kat = $this->kat($stok);
    $sql = "INSERT INTO tbl_barang (kd_barang, nm_barang, harga, jml, stok, kat) VALUESS ('$kd_barang', '$nm_barang', '$harga', '$jml', '$stok', '$kat')";
    $this->conn->query($sql);
}
...
{
    $sql = "SELECT * FROM tbl_barang WHERE kd_barang ='$id'";
    $bind = $this->conn->query($sql);
    while($obj = $bind->fetch_object()) {
        $baris = $obj;
    }
...
// tbl_keluar
class Model2 extends connection
{
    public function __construct()
{
    $this->conn = $this->get_connection();
